...
from src.model_components import SubwordEmbedding, SelfAttention, circulant_mask, FeedForward1, SharedInnerBlock, MultiheadAttention, TransformerLayers, FeedForward2
import pytest
...
# pytest -s tests/test_model_components.py make sure PYTHONPATH=.
...
def test_SubwordEmbedding(train_config,model_config):
    ...
    input_token_ids = torch.randint(
        high = model_config.vocab_size, size = (train_config.train_batch_size, SEQ_LEN)
    )
    positional_input = torch.randint(
        high = model_config.vocab_size, size = (train_config.train_batch_size, SEQ_LEN)
    )

    embedding_output = embedding(
        input_ids = input_token_ids, position_ids = positional_input
    )

    assert embedding_output.size() == (
...
def test_FeedForward1(train_config,model_config):
    ff1 = FeedForward1(
        model_config.num_embed_hidden, model_config.feed_forward1_hidden, model_config.dropout_rate
    )
